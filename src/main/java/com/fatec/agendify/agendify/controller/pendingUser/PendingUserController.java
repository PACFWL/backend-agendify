...
public class PendingUserController {
   ...
   @GetMapping("/pending/{id}")
    ...
    public ResponseEntity<Map<String, String>> registerPendingUser(@Valid @RequestBody UserCreateDTO userDTO) {
        PendingUser pending = pendingUserService.createPendingUser(userDTO);
    ...
    @PreAuthorize("hasRole('MASTER')")
    public ResponseEntity<PendingUser> updatePendingUser(@PathVariable String id, @Valid @RequestBody UserCreateDTO userDTO) {
        return ResponseEntity.ok(pendingUserService.updatePendingUser(id, userDTO));
    }
